                ...
                select new Ship
                {
                    ...
                    Characters =
                        new ObservableCollection<Character>(
                            FindCharacters.ReadCharacters(shipNode.SelectSingleNode(".//characters"))),
                    StorageFacilities = new ObservableCollection<StorageFacility>(
                        FindStorages.ReadStorageFacilities(shipNode.SelectNodes(".//feat[@eatAllowed]"))),
                    ToolFacilities =
                        new ObservableCollection<ToolFacility>(
                            FindStorages.ReadToolFacilities(shipNode.SelectNodes(".//feat[@ft]")))
                }).ToList();
